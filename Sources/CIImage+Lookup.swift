...
public extension CIImage {
    ...
    ///   - options: A dictionary that contains options for creating the image object.
    ...
    convenience init?(named name: String, in bundle: Bundle? = nil, options: [CIImageOption: Any]? = nil, compatibleWith traitCollection: UITraitCollection? = nil) {
        ...
        if let uiImage = UIImage(named: name, in: bundle, compatibleWith: traitCollection) {
            self.init(image: uiImage, options: options)
        } else {
            return nil
        }
    }
    ...
    ///   - bundle: The bundle containing the image file or asset catalog. Specify nil to search the app’s main bundle.
    ///   - options: A dictionary that contains options for creating the image object.
    convenience init?(named name: String, in bundle: Bundle? = nil, options: [CIImageOption: Any]? = nil) {
